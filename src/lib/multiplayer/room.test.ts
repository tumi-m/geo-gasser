...
describe("server-authoritative room", () => {
  ...
  it("rematch auto-starts when both players are still connected", () => {
    ...
    const live = s;
    assert.equal(applyRoomCommand(s, cmd({ t: "rematch", playerId: "host", seed: 99 })), live);
    while (s.phase !== "final_reveal") {
      s = applyRoomCommand(s, cmd({ t: "continue", playerId: "host", now: now + 50_000 }));
      if (s.phase === "final_reveal") break;
      s = applyRoomCommand(s, cmd({ t: "intro", playerId: "host", now: now + 50_000 }));
      s = applyRoomCommand(s, cmd({ t: "timeout", now: roundDeadlineMs(s)! }));
    }
    s = applyRoomCommand(s, cmd({ t: "rematch", playerId: "host", seed: 99, now: now + 100_000 }));
    assert.equal(s.phase, "round_intro");
