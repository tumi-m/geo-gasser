...
test('five-stop classic is five distinct stops with a worldwide finale',()=>{
 for(let seed=0;seed<30;seed++){
 const p=planMatch(seed,'escape');assert.equal(p.totalRounds,5);assert.equal(new Set(p.locationIds).size,5);
...
test('custom city filter never silently widens the map',()=>{
 const p=planMatch(3,'escape',{preset:'za',nations:['ZA'],cities:['Cape Town']});
...
test('time origin zero works, invalid pins and late locks cannot earn points',()=>{
 let s=reduce(createLobbyState(),{type:'CREATE_SOLO',playerId:'me',name:'Me',seed:1,now:0,matchLength:'escape'});
...
test('hotseat timeout hands over, and second timer never alters first response',()=>{
 let s=reduce(createLobbyState(),{type:'CREATE_LOCAL_DUEL',seats:[{id:'a',name:'A'},{id:'b',name:'B'}],hotseat:true,seed:1,now:1,matchLength:'escape'});
 ...
 let t=reduce(createLobbyState(),{type:'CREATE_LOCAL_DUEL',seats:[{id:'a',name:'A'},{id:'b',name:'B'}],hotseat:true,seed:1,now:1,matchLength:'escape'});
