...
export function grokGuess(loc: GeoLocation, seed: number, roundIndex: number): LatLng {
  ...
  if (mixup) {
    return loc.country === "ZA"
      ? offsetKm({ latitude: 52.09, longitude: 5.12 }, 8 + rand() * 70, rand() * 360)
      : offsetKm({ latitude: -28.7, longitude: 24.8 }, 40 + rand() * 400, rand() * 360);
  }
  const table = loc.country === "NL" ? NL_SCALE : ZA_SCALE;
