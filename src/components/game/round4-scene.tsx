...
import { ATMOSPHERE_FAMILY, type EnvironmentSpec } from "@/lib/game";
...
export function Round4Scene({ env, reducedMotion }: { env: EnvironmentSpec; reducedMotion?: boolean }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const family = ATMOSPHERE_FAMILY[env.atmosphere] ?? "cape";
      const scene = new THREE.Scene();
      scene.fog = new THREE.Fog(family === "canal" || family === "harbor" ? 0x070814 : 0x120c08, 8, 42);
      const camera = new THREE.PerspectiveCamera(60, 1, 0.1, 200);
      camera.position.set(0, 3.2, 10);

      renderer = new THREE.WebGLRenderer({ antialias: true, powerPreference: "high-performance" });
      renderer.setPixelRatio(Math.min(window.devicePixelRatio, 2));
      renderer.setClearColor(family === "cape" || family === "veld" ? 0x1a140e : 0x0b0c14);
      ...
      const ground = new THREE.Mesh(
        ...
        new THREE.MeshStandardMaterial({
          color: family === "canal" || family === "harbor" ? 0x12151c : 0x2a241c,
          roughness: 0.95,
        }),
      );
      ...
      if (family !== "veld") {
        const water = new THREE.Mesh(
          new THREE.PlaneGeometry(family === "cape" ? 80 : 8, family === "cape" ? 28 : 48),
          new THREE.MeshStandardMaterial({
            color: family === "canal" ? 0x1b2433 : 0x1a2a38,
            roughness: 0.2,
            metalness: 0.4,
          }),
        );
        ...
        water.position.z = family === "cape" ? -12 : 0;
        scene.add(water);
      }

      const buildingMat = new THREE.MeshStandardMaterial({
        color: family === "canal" ? 0x3a2a22 : 0x4a4338,
        roughness: 0.8,
      });
      const count = window.innerWidth < 600 ? 18 : 36;
      for (let i = 0; i < count; i++) {
        const h = 0.8 + Math.random() * (family === "harbor" ? 4 : 2.2);
        const mesh = new THREE.Mesh(new THREE.BoxGeometry(0.8, h, 0.9), buildingMat);
        const side = i % 2 === 0 ? -3.2 : 3.2;
        mesh.position.set(
          family === "cape" ? (Math.random() - 0.5) * 16 : side + (Math.random() - 0.5) * 0.4,
          h / 2,
          (Math.random() - 0.5) * 22,
        );
        scene.add(mesh);
      ...
      const key = new THREE.DirectionalLight(family === "canal" ? 0x88a0ff : 0xffc888, 1.6);
  ...
  }, [env.id, env.atmosphere, env.backdropUrl, reducedMotion]);
